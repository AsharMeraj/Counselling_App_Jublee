...
import { epdsQuestions } from "@/app/_utils/scales/epdsQuestions";
import { urduEpdsQuestions } from "@/app/_utils/scales/epdsQuestions"
...
const EPDSForm = () => {
  ...
  const currentQuestions = isUrdu ? urduEpdsQuestions.questions : epdsQuestions.questions;
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch("/api/save-result", {
        ...
        body: JSON.stringify({ 
          entryId, 
          questionnaireType: isUrdu ? urduEpdsQuestions.type : epdsQuestions.type, 
          totalScore 
        }),
      });
    ...
    <div className=" bg-(--primary)/5 pb-10 px-6 md:px-12 pt-10 md:pt-10">
      ...
      <div className="text-center my-10 flex flex-col md:flex-row justify-center items-center gap-4">
        <h1 className="text-[20px] md:text-[25px] font-bold text-secondary">
          {isUrdu ? "ایڈنبرا بعد از پیدائش ڈپریشن سوالنامہ" : "EPDS Questionnaire"}
        </h1>
        <button
          type="button"
