...
const WorkStressForm = () => {
  ...
  const handleSubmit = async () => {
    if (Object.keys(answers).length < workStressQuestions.questions.length) {
      alert(`Please answer all ${workStressQuestions.type} questions before submitting.`);
      return;
    }
    ...
    try {
      // Save the result dynamically based on questionnaire type
      await fetch("/api/save-result", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ entryId, questionnaireType: workStressQuestions.type, totalScore }),
      });
  ...
  const progress = Math.round(
    (Object.keys(answers).length / workStressQuestions.questions.length) * 100
  );
    ...
    <div className="min-h-[calc(100vh-13.5rem)] bg-(--primary)/5 pb-10 px-6 md:px-12 pt-10 md:pt-10">
        ...
        <div className="flex justify-between items-center mb-2">
          <span className="text-(--primary) font-semibold">
            ...
            <span className="text-(--secondary)">
              {workStressQuestions.questions.length - Object.keys(answers).length}
            </span>
          </span>
          <span className="text-(--primary) font-semibold">
      ...
      <div className="text-center my-10">
        <h1 className="text-[20px] md:text-[25px] font-bold text-(--secondary)">
          {workStressQuestions.type} Questionnaire
        </h1>
      </div>

      {workStressQuestions.questions.map((q, i) => (
